...
#   openings  [{id, type: "door"|"window", wall, pos, width, swing, hinge, height, kind (door: exterior|interior|garage|sliding), sill (window)}]  (pos = ft from wall start)
#   labels    [{id, text, x, y, size}]
#   fixtures  [{id, kind, x, y, w, h, rot, label, dir (stairs), hgt (columns, chimneys)}]   (see FIXTURES in the JS)
#   guides    [{id, x1, y1, x2, y2, label}]   reference-only dashed lines (setbacks, distances); never rendered in 3D
#   roofs     [{id, x, y, w, h, style hip|gable|shed|flat|gambrel|mansard, pitch, pitchB, shift, ridge auto|x|y, endA/endB hip|gable|dutch,
#               overhang, rake, eave, ridgeH, color, high n|s|w|e, dormers [{type gable|shed, side a|b, pos, w, setback, wall, pitch}]}]
#             roof sections drawn with the Roof tool; they replace the automatic roof in 3D
#   decks     [{id, x, y, w, h, height, material, rails {n,s,e,w}, stairs none|n|s|e|w, stairW}]
class Design::FloorPlan < ApplicationRecord
  belongs_to :concept, class_name: "Design::Concept"
  has_one_attached :thumbnail

  DEFAULT_DATA = { "version" => 1, "grid" => 0.5, "walls" => [], "rooms" => [], "openings" => [], "labels" => [], "fixtures" => [], "guides" => [], "roofs" => [], "decks" => [] }.freeze
  SECTIONS = %w[walls rooms openings labels fixtures guides roofs decks].freeze
